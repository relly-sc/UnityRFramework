...
namespace UnityRFramework.Runtime
{
    ...
    /// </summary>
    public sealed class DefaultConfigHelper : DictionaryConfigHelperBase
    {
        /// <inheritdoc/>
        public override object ParseConfig(Type tableType, byte[] bytes)
        {
            ...
            {
                throw new RFrameworkException("DefaultConfigHelper: JSON bytes are empty.");
            }

            string json = Encoding.UTF8.GetString(bytes);
            if (string.IsNullOrWhiteSpace(json))
            {
                throw new RFrameworkException("DefaultConfigHelper: decoded JSON content is empty.");
            }
