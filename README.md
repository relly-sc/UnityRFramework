...
框架没有独立 DataModule，配置数据统一由 ConfigModule 管理。零第三方 Editor 转换工具位于菜单 `UnityRFramework/配置表工具`：Config CSV 前三行依次为字段名、类型、注释，必须包含唯一 `int Id`；Localization CSV 使用 `Key,Value`。工具生成配置行、静态 Codec、URFC v2 和 URFL v1，并仅在内容变化时写入。Config 与 Localization 的二进制输出目录必须分开，避免同名文件相互覆盖。
...
| 配置表 | `DefaultConfigHelper`（JSON）/ `BinaryConfigHelper`（URFC v1/v2） | Luban、自定义二进制/文本格式 |
| 本地化 | `DefaultLocalizationHelper`（JSON）/ `BinaryLocalizationHelper`（URFL v1） | 自定义二进制/文本格式 |
