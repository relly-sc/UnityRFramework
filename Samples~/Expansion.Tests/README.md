# UnityRFramework · Expansion.Tests

`Expansion.Tests` 是第三方 Helper 的最小端到端验收场景，不复制正式 Demo 的业务
...
`Expansion.Tests` 不能在刚导入后直接进入 Play Mode。UPM 的 Import Sample 只复制
Sample 目录，不允许 Sample 在导入过程中自动向宿主工程
`Assets/StreamingAssets` 写文件，因此必须完成以下准备：

1. 先导入 `Expansion.YooAsset` 与 `Expansion.UniTask` Sample，再导入 `Expansion.Tests` Sample。
...
`Assets/UnityRFramework/Samples/Expansion.Tests` 和 Package Manager 导入后的
`Assets/Samples/<包名>/<版本>/Expansion.Tests`，不要求用户移动 Sample。
