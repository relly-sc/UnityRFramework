# UnityRFramework · ExpansionAcceptance

`ExpansionAcceptance` 是第三方 Helper 的最小端到端验收场景，不复制正式 Demo 的业务
...
`ExpansionAcceptance` 不能在刚导入后直接进入 Play Mode。UPM 的 Import Sample 只复制
Sample 目录，不允许 Sample 在导入过程中自动向宿主工程
`Assets/StreamingAssets` 写文件，因此必须完成以下准备：

1. 先导入 `Expansion` Sample，再导入 `ExpansionAcceptance` Sample。
...
`Assets/UnityRFramework/Samples/ExpansionAcceptance` 和 Package Manager 导入后的
`Assets/Samples/<包名>/<版本>/ExpansionAcceptance`，不要求用户移动 Sample。
