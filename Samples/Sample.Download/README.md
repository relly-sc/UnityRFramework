...
2. 确认场景中的 `UnityRFramework` 预制体包含 `WebRequestComponent` 和
   `DownloadComponent`。
3. 进入 Play Mode，在界面中填写文件 URL 与保存文件名。
...
| 成功后解压 ZIP | 下载提交后执行安全 ZIP 解压 |
| 解压目录 | 验收目录下的目标子目录 |
| 解压后删除压缩包 | 仅在解压成功后删除 ZIP |
...
1. 准备一个可访问的 HTTP 文件，填写 URL 和保存文件名。
2. 点击“重新下载”。
...
1. 下载 `.zip` 文件，启用“成功后解压 ZIP”。
2. 填写解压目录并开始下载。
3. 确认解压目录生成且内容完整。
4. 分别验证保留压缩包和“解压后删除压缩包”两种设置。

解压期间进度条显示解压后字节进度和当前条目。默认实现使用
`System.IO.Compression`，在后台线程执行纯文件 I/O。项目可通过
`GameEntry.Download.SetArchiveHelper(IArchiveHelper)` 注入 SharpZipLib 等第三方实现；
...
- 普通下载、取消、继续、大小校验、SHA-256 校验和 ZIP 解压行为符合界面状态。
